<?php
				...
				try{	
					...
					try{					
						...
						if ($insert_user_result){
							...
							foreach (json_decode((string) $request->getParsedBody()['bevorzugte_dienstleister']) as $f){
    ...
    $app->put('/kunden/user/{id}', function($request, $response, $args) {
        ...
        if(filter_var($request->getParsedBody()['debug'], FILTER_VALIDATE_BOOLEAN)){
            ...
            $user_info = ["ID"            => $user_id, "user_email"    => $email, "display_name"  => $request->getParsedBody()['titel'].' '.$request->getParsedBody()['vorname'].' '.$request->getParsedBody()['nachname'], "first_name"    => $request->getParsedBody()['vorname'], "last_name"     => $request->getParsedBody()['nachname'], "role"          => "kunde_u"];
