...
function twentysixteen_custom_header_and_background() {
	...
	$default_background_color = trim( (string) $color_scheme[0], '#' );
	$default_text_color       = trim( (string) $color_scheme[3], '#' );
	 ...
	 * }
	 */
	add_theme_support( 'custom-background', apply_filters( 'twentysixteen_custom_background_args', ['default-color' => $default_background_color] ) );
	 ...
	 * }
	 */
	add_theme_support( 'custom-header', apply_filters( 'twentysixteen_custom_header_args', ['default-text-color'     => $default_text_color, 'width'                  => 1200, 'height'                 => 280, 'flex-height'            => true, 'wp-head-callback'       => 'twentysixteen_header_style'] ) );
}
...
function twentysixteen_customize_register( $wp_customize ) {
	...
	if ( isset( $wp_customize->selective_refresh ) ) {
		$wp_customize->selective_refresh->add_partial( 'blogname', ['selector' => '.site-title a', 'container_inclusive' => false, 'render_callback' => 'twentysixteen_customize_partial_blogname'] );
		$wp_customize->selective_refresh->add_partial( 'blogdescription', ['selector' => '.site-description', 'container_inclusive' => false, 'render_callback' => 'twentysixteen_customize_partial_blogdescription'] );
	}

	// Add color scheme setting and control.
	$wp_customize->add_setting( 'color_scheme', ['default'           => 'default', 'sanitize_callback' => 'twentysixteen_sanitize_color_scheme', 'transport'         => 'postMessage'] );

	$wp_customize->add_control( 'color_scheme', ['label'    => __( 'Base Color Scheme', 'twentysixteen' ), 'section'  => 'colors', 'type'     => 'select', 'choices'  => twentysixteen_get_color_scheme_choices(), 'priority' => 1] );

	// Add page background color setting and control.
	$wp_customize->add_setting( 'page_background_color', ['default'           => $color_scheme[1], 'sanitize_callback' => 'sanitize_hex_color', 'transport'         => 'postMessage'] );

	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'page_background_color', ['label'       => __( 'Page Background Color', 'twentysixteen' ), 'section'     => 'colors'] ) );

	// Remove the core header textcolor control, as it shares the main text color.
	$wp_customize->remove_control( 'header_textcolor' );

	// Add link color setting and control.
	$wp_customize->add_setting( 'link_color', ['default'           => $color_scheme[2], 'sanitize_callback' => 'sanitize_hex_color', 'transport'         => 'postMessage'] );

	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'link_color', ['label'       => __( 'Link Color', 'twentysixteen' ), 'section'     => 'colors'] ) );

	// Add main text color setting and control.
	$wp_customize->add_setting( 'main_text_color', ['default'           => $color_scheme[3], 'sanitize_callback' => 'sanitize_hex_color', 'transport'         => 'postMessage'] );

	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'main_text_color', ['label'       => __( 'Main Text Color', 'twentysixteen' ), 'section'     => 'colors'] ) );

	// Add secondary text color setting and control.
	$wp_customize->add_setting( 'secondary_text_color', ['default'           => $color_scheme[4], 'sanitize_callback' => 'sanitize_hex_color', 'transport'         => 'postMessage'] );

	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'secondary_text_color', ['label'       => __( 'Secondary Text Color', 'twentysixteen' ), 'section'     => 'colors'] ) );
}
...
function twentysixteen_get_color_schemes() {
	 ...
	 * }
	 */
	return apply_filters( 'twentysixteen_color_schemes', ['default' => ['label'  => __( 'Default', 'twentysixteen' ), 'colors' => ['#1a1a1a', '#ffffff', '#007acc', '#1a1a1a', '#686868']], 'dark' => ['label'  => __( 'Dark', 'twentysixteen' ), 'colors' => ['#262626', '#1a1a1a', '#9adffd', '#e5e5e5', '#c1c1c1']], 'gray' => ['label'  => __( 'Gray', 'twentysixteen' ), 'colors' => ['#616a73', '#4d545c', '#c7c7c7', '#f2f2f2', '#f2f2f2']], 'red' => ['label'  => __( 'Red', 'twentysixteen' ), 'colors' => ['#ffffff', '#ff675f', '#640c1f', '#402b30', '#402b30']], 'yellow' => ['label'  => __( 'Yellow', 'twentysixteen' ), 'colors' => ['#3b3721', '#ffef8e', '#774e24', '#3b3721', '#5b4d3e']]] );
}
...
function twentysixteen_color_scheme_css() {
	...
	$colors = ['background_color'      => $color_scheme[0], 'page_background_color' => $color_scheme[1], 'link_color'            => $color_scheme[2], 'main_text_color'       => $color_scheme[3], 'secondary_text_color'  => $color_scheme[4], 'border_color'          => vsprintf( 'rgba( %1$s, %2$s, %3$s, 0.2)', $color_textcolor_rgb )];
...
function twentysixteen_customize_preview_js() {
	wp_enqueue_script( 'twentysixteen-customize-preview', get_template_directory_uri() . '/js/customize-preview.js', ['customize-preview'], '20160412', true );
}
...
function twentysixteen_get_color_scheme_css( $colors ) {
	$colors = wp_parse_args( $colors, ['background_color'      => '', 'page_background_color' => '', 'link_color'            => '', 'main_text_color'       => '', 'secondary_text_color'  => '', 'border_color'          => ''] );
...
function twentysixteen_color_scheme_css_template() {
	$colors = ['background_color'      => '{{ data.background_color }}', 'page_background_color' => '{{ data.page_background_color }}', 'link_color'            => '{{ data.link_color }}', 'main_text_color'       => '{{ data.main_text_color }}', 'secondary_text_color'  => '{{ data.secondary_text_color }}', 'border_color'          => '{{ data.border_color }}'];
	?>
