	...
	private function readPropertyValue(Customweb_Xml_Binding_Reflection_Property $reflector, DOMNode $parentNode) {
		...
		if ($reflector->isAttribute()) {
			if ($parentNode instanceof DOMElement) {
				if (!empty($namespaceUri)) {
					if ($parentNode->hasAttributeNS($namespaceUri, $reflector->getName())) {
						return $this->instantiateSimpleType($reflector->getItemPhpClasName(), $parentNode->getAttributeNS($namespaceUri, $reflector->getName()));
					}
					else {
						return null;
					}
				...
				throw new Exception(Customweb_Core_String::_("Unable to read attribute @name.")->format(array('@name' => $reflector->getName())));
			}
		}
			...
			if ($parentNode instanceof DOMElement) {
				...
				throw new Exception(Customweb_Core_String::_("Unable to read element @name.")->format(array('@name' => $reflector->getName())));
			}
		}
			...
			if ($parentNode instanceof DOMElement) {
				...
				if ($nodeList->length > 0) {
					return $this->instantiateSimpleType($reflector->getItemPhpClasName(), $nodeList->item(0)->nodeValue);
				}
				else {
					return null;
				}
			...
			if ($parentNode instanceof DOMElement) {
				...
				if ($nodeList->length > 0) {
					return $this->decodeComplexType($nodeList->item(0), $reflector->getItemPhpClasName());
				}
				else {
					return null;
				}
