...
class Mpay24Cw_CartUtil {
	...
	public static function getInoviceItemsFromCart(WC_Cart $cart){
		...
		foreach ($wooCommerceItems as $wooItem) {
			...
			if($discountAmount > 0){
				$discountItem = new Customweb_Payment_Authorization_DefaultInvoiceItem($sku.'-discount', __("Discount", "woocommerce_mpay24cw").' '.$name, $taxRate, $discountAmount, 1,
		...
		if ($cart->shipping_total > 0) {
			...
			$items[] = new Customweb_Payment_Authorization_DefaultInvoiceItem('shipping', __("Shipping", "woocommerce_mpay24cw"), $taxRate, $shippingExclTax + $shippingTax, 1, 
