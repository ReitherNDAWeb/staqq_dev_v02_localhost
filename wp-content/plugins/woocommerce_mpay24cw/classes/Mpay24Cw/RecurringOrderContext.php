...
require_once 'Mpay24Cw/AbstractRecurringOrderContext.php';
require_once 'Mpay24Cw/CartUtil.php';


class Mpay24Cw_RecurringOrderContext extends Mpay24Cw_AbstractRecurringOrderContext
{
	protected $productId;
	...
	public function __construct($order, $paymentMethod, $amountToCharge, $productId) {
		...
		if(!empty($initialTransactionId)){
			...
			$initialTransaction = Mpay24Cw_Util::getTransactionById($initialTransactionId);
		}
		else{
			$this->setInitialOrderId($order->id);
			$initialTransaction = Mpay24Cw_Util::getAuthorizedTransactionByPostId($this->getInitialOrderId());
			if(empty($initialTransaction)){
				$initialTransaction = Mpay24Cw_Util::getAuthorizedTransactionByOrderId($this->getInitialOrderId());
			}
		}
	...
	public function getInvoiceItems() {
		...
		if ($cartDiscount > 0) {
			$items = Mpay24Cw_CartUtil::applyCartDiscounts($cartDiscount, $items);
		}
		...
		if ($orderDiscount > 0) {
			...
			$items[] = new Customweb_Payment_Authorization_DefaultInvoiceItem(
				'order-discount',
				__('Discount', 'woocommerce_mpay24cw'),
				$taxRate,
				$orderDiscount,
				1,
		...
		if ($difference > 0) {
			...
			$items[] = new Customweb_Payment_Authorization_DefaultInvoiceItem(
				'other-discount',
				__('Other Discount', 'woocommerce_mpay24cw'),
				$taxRate,
				abs($difference),
				1,
				Customweb_Payment_Authorization_DefaultInvoiceItem::TYPE_DISCOUNT
