...
class Mpay24Cw_Migration_1_1_0 implements Customweb_Database_Migration_IScript {

	public function execute(Customweb_Database_IDriver $driver){
		global $wpdb;	

		$query = "SHOW TABLES IN `".DB_NAME."` LIKE  'mpay24cw_storage';";
		...
		if(is_array($select) && count($select) > 0) {
			...
			$tableName = $entityManager->getTableNameForEntityByClassName('Mpay24Cw_Entity_Storage');
			$driver->query("RENAME TABLE `mpay24cw_storage` TO `" . $tableName . "`")->execute();
		}
		else {
			$driver->query(
			
				"CREATE TABLE IF NOT EXISTS `".$wpdb->prefix . "woocommerce_mpay24cw_storage` (
