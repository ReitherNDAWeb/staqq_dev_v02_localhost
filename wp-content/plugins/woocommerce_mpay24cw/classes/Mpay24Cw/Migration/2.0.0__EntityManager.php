...
class Mpay24Cw_Migration_2_0_0 implements Customweb_Database_Migration_IScript {

	public function execute(Customweb_Database_IDriver $driver){
		global $wpdb;
		
		$tableName = $wpdb->prefix . 'mpay24cw_transactions';
		...
		$tableNameNew = $entityManager->getTableNameForEntityByClassName('Mpay24Cw_Entity_Transaction');
		
		$driver->query("RENAME TABLE `" . $tableName . "` TO `" . $tableNameNew . "`")->execute();
		
		$driver->query(
				"CREATE TABLE IF NOT EXISTS `" . $wpdb->prefix . "woocommerce_mpay24cw_customer_contexts` ( 
