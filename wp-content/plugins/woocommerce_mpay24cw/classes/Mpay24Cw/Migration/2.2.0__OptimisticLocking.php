...
class Mpay24Cw_Migration_2_2_0 implements Customweb_Database_Migration_IScript {
	...
	public function execute(Customweb_Database_IDriver $driver){
		...
		$tableNameECC = $entityManager->getTableNameForEntityByClassName('Mpay24Cw_Entity_ExternalCheckoutContext');
		$driver->query("ALTER TABLE `" . $tableNameECC . "` ADD COLUMN  `versionNumber` int NOT NULL")->execute();
		
		$tableNamePCC = $entityManager->getTableNameForEntityByClassName('Mpay24Cw_Entity_PaymentCustomerContext');
