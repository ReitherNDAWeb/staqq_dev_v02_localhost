 ...
 *
 */
class Mpay24Cw_Controller_Success extends Mpay24Cw_Controller_Abstract {
	...
	public function indexAction() {
		...
		$method = Mpay24Cw_Util::getPaymentMehtodInstance($dbTransaction->getPaymentClass());
		if (method_exists($method, 'get_return_url')) {
			$successUrl = $method->get_return_url($order);
		}
		else {
			$option = Mpay24Cw_Util::getShopOption('woocommerce_thanks_page_id');
			...
			$successUrl = add_query_arg('key', $order->order_key, add_query_arg('order', $order->id, get_permalink(Mpay24Cw_Util::getPermalinkIdModified($checkout_redirect))));
		}
		...
		while (true) {
			...
			if ($transactionObject->isAuthorizationFailed()) {
				...
				$url = Mpay24Cw_Util::getPluginUrl('failure', array('cwtid' => $parameters['cwtid'], 'cwtt' => $parameters['cwtt']));
			}
			...
			if (time() - $start > $maxExecutionTime) {
				ob_start();
				$GLOBALS['woo_mpay24cwTitle'] = __('Time Out' , 'woocommerce_mpay24cw');
				Mpay24Cw_Util::includeTemplateFile('timeout', array('successUrl' => $successUrl));
