...
require_once 'Mpay24Cw/AbstractPaymentMethod.php';
require_once 'Mpay24Cw/RecurringOrderContextNew.php';
require_once 'Customweb/Core/Stream/Input/File.php';
require_once 'Customweb/Core/Http/ContextRequest.php';
require_once 'Mpay24Cw/RecurringTransactionContext.php';
require_once 'Mpay24Cw/RecurringOrderContext.php';
 ...
 * Mpay24Cw server.
 */
class Mpay24Cw_PaymentMethod extends Mpay24Cw_AbstractPaymentMethod {
	...
	public function __construct(){
		...
		// Workaround: When some setting is stored all Mpay24Cw methods are
		...
		if ($this->getPaymentMethodConfigurationValue('enabled') == 'yes') {
			$adapter = Mpay24Cw_Util::getAuthorizationAdapter(
	...
	public function admin_options(){
		$output = '<h3>' . __($this->admin_title, 'woocommerce_mpay24cw') . '</h3>';
	...
	public function scheduledSubscriptionPayment($amountToCharge, $order, $productId){
		...
		$mpay24cw_recurring_process_failure = NULL;
		try {
			$adapter = Mpay24Cw_Util::getAuthorizationAdapter(
					Customweb_Payment_Authorization_Recurring_IAdapter::AUTHORIZATION_METHOD_NAME);
			
			$orderContext = new Mpay24Cw_RecurringOrderContext($order, new Mpay24Cw_PaymentMethodWrapper($this), $amountToCharge, 
					$productId);
			$dbTransaction = $this->newDatabaseTransaction($orderContext);
			$transactionContext = new Mpay24Cw_RecurringTransactionContext($dbTransaction, $orderContext);
			...
			$errorMessage = __('Subscription Payment Failed with error:', 'woocommerce_mpay24cw') . $e->getMessage();
			$mpay24cw_recurring_process_failure = $errorMessage;
			WC_Subscriptions_Manager::process_subscription_payment_failure_on_order($order, $product_id);
			return;
		}
		try {
			$adapter->process($transaction);
			Mpay24Cw_Util::getTransactionHandler()->persistTransactionObject($transaction);
	...
	public function scheduledSubscriptionPaymentNew($amountToCharge, $order){
		global $mpay24cw_recurring_process_failure;
		$mpay24cw_recurring_process_failure = NULL;
		if(Mpay24Cw_Util::getAuthorizedTransactionByPostId($order->id) != null){
			return;	
		}
		try {			
			$adapter = Mpay24Cw_Util::getAuthorizationAdapter(
			...
			$orderContext = new Mpay24Cw_RecurringOrderContextNew($order, new Mpay24Cw_PaymentMethodWrapper($this), 
					$amountToCharge);
			$order->update_status('wc-pend-'.substr(hash('sha1', 'mpay24cw'), 0 , 10));
			$dbTransaction = $this->newDatabaseTransaction($orderContext);
			$transactionContext = new Mpay24Cw_RecurringTransactionContext($dbTransaction, $orderContext);
			$transaction = $adapter->createTransaction($transactionContext);
			$dbTransaction->setTransactionObject($transaction);
	...
	public function process_admin_options(){
		global $woocommerce_mpay24cw_isProcesssing;
		if ($woocommerce_mpay24cw_isProcesssing) {
			return true;
		}
		$woocommerce_mpay24cw_isProcesssing = true;
	...
	public function validate_file_field($key){
		...
		if (!file_exists($upload_dir['basedir'] . '/woocommerce_mpay24cw')) {
			$oldmask = umask(0);
			mkdir($upload_dir['basedir'] . '/woocommerce_mpay24cw', 0777, true);
			umask($oldmask);
		}
		...
		$targetPath = $upload_dir['basedir'] . '/woocommerce_mpay24cw/' . $filename . '.' . $fileExtension;
		...
		if ($rs) {
			...
			woocommerce_mpay24cw_admin_show_message('Unable to upload file for setting "' . $setting['title'] . '".', 'error');
			return $value;
		}
	}
...
<tr valign="top">
	...
	<td class="forminp">
		...
		echo __('Current File: ', 'woocommerce_mpay24cw') . esc_attr($filename);
		?><br />
		<fieldset>
		...
		name="<?php echo esc_attr( $field.'_reset' ); ?>" value="reset" /><?php echo __('Reset', 'woocommerce_mpay24cw'); ?><br />
	</td>
</tr>
<?php
		return ob_get_clean();
	...
	protected function getOrderStatusOptions($statuses = array()){
		if (defined('WOOCOMMERCE_VERSION') && version_compare(WOOCOMMERCE_VERSION, '2.2.0') >= 0) {
			...
			foreach ($statuses as $k => $value) {
				$orderStatuses[$k] = __($value, 'woocommerce_mpay24cw');
			}
	...
	protected function getCompatibilityFormFields(){
		...
		if (is_plugin_active('woocommerce-german-market/WooCommerce-German-Market.php')) {
			$extra .= '<div class="mpay24cw-requires-second-run"></div>';
		}
		return $extra;
	}
	...
	public function process_payment($order_id){
		...
		$order = Mpay24Cw_Util::loadOrderObjectById($order_id);
		...
		$order->add_order_note(
				__('The customer is now in the payment process of mPAY24.', 'woocommerce_mpay24cw'));
		$order->update_status('wc-pend-'.substr(hash('sha1', 'mpay24cw'), 0 , 10));
		...
		if (is_ajax()) {
				...
				if (is_array($result)) {
					...
					wp_send_json(
							array(
								'result' => 'success',
								'data' => $result .
										 "<script type=\"text/javascript\"> var backToCheckoutCw = jQuery('#mpay24cw-back-to-checkout'); jQuery('form.checkout').replaceWith(jQuery('#mpay24cw-payment-container')); jQuery('#mpay24cw-payment-container').after(backToCheckoutCw); jQuery('.woocommerce-info').remove(); jQuery('.cw-external-checkouts').remove(); jQuery('html, body').animate({ scrollTop: (jQuery('#mpay24cw-payment-container').offset().top-150) }, '1000');</script>" 
							));
					die();
				}
			}
			...
			wp_redirect(
					Mpay24Cw_Util::getPluginUrl("payment", 
							array(
								'cwoid' => $order_id,
								'cwot' => Mpay24Cw_Util::computeOrderValidationHash($order_id),
	...
	protected function destroyCheckoutId(){
		...
		if($sessionHandler != null){
			if(method_exists($sessionHandler, 'set')){
				$sessionHandler->set('Mpay24CwCheckoutId', null);
			}
			else{
				$sessionHandler->Mpay24CwCheckoutId = null;
			}
		}
	}
