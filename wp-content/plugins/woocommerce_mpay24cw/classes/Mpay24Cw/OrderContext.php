...
require_once 'Mpay24Cw/AbstractOrderContext.php';
...
class Mpay24Cw_OrderContext extends Mpay24Cw_AbstractOrderContext {
	...
	public function __construct($order, Customweb_Payment_Authorization_IPaymentMethod $paymentMethod) {
		...
		if($sessionHandler != null){
			if(method_exists($sessionHandler, 'get')){
				$checkoutId = $sessionHandler->get('Mpay24CwCheckoutId', null);
				if($checkoutId === null) {
					$checkoutId = Customweb_Core_Util_Rand::getUuid();
					$sessionHandler->set('Mpay24CwCheckoutId', $checkoutId);
				}
			}
			else{
				$checkoutId = $sessionHandler->Mpay24CwCheckoutId;
				if($checkoutId === null) {
					$checkoutId = Customweb_Core_Util_Rand::getUuid();
					$sessionHandler->Mpay24CwCheckoutId = $checkoutId;
				}
			}
		}
		else {
	...
	protected function getInvoiceItemsInternal(){
		...
		foreach ($wooCommerceItems as $wooItem) {
			...
			if($discountAmount > 0){
				$discountItem = new Customweb_Payment_Authorization_DefaultInvoiceItem($sku.'-discount', __("Discount", "woocommerce_mpay24cw").' '.$wooItem['name'], $taxRate, $discountAmount, 1,
	...
	public function getShippingMethod(){
		...
		if(count($wooCommerceShipping) == 0){
			return __("Free Shipping", "woocommerce_mpay24cw");
		}
		if(count($wooCommerceShipping) > 1){
			return __("Multiple Shipping Methods", "woocommerce_mpay24cw");
		}
