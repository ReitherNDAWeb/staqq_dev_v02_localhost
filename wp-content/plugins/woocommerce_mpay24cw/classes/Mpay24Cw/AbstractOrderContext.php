...
abstract class Mpay24Cw_AbstractOrderContext extends  Customweb_Payment_Authorization_OrderContext_AbstractDeprecated
{
	protected $order;
	...
	public function __construct($order, Customweb_Payment_Authorization_IPaymentMethod $paymentMethod) {
		...
		elseif (function_exists('get_woocommerce_currency')) {
			...
			$this->currencyCode = Mpay24Cw_Util::getShopOption('woocommerce_currency');
		}
		$this->order = $order;
	...
	public function getOrderNumber(){
		$orderNumber = null;
		if(Mpay24Cw_ConfigurationAdapter::getOrderNumberIdentifier() == 'ordernumber') {		
			$orderNumber = preg_replace('/[^a-zA-Z\d]/', '', $this->order->get_order_number());
		}
		else{
			$orderNumber = $this->order->id;
		}
		$existing = Mpay24Cw_Util::getTransactionsByOrderId($orderNumber);
		if(count($existing) > 0) {
			$number  = count(Mpay24Cw_Util::getTransactionsByPostId($this->getOrderPostId()));
