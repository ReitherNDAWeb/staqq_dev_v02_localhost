 ...
 *
 *
 * @Entity(tableName = 'woocommerce_mpay24cw_transactions')
 * @Filter(name = 'loadByPostId', where = 'postId = >postId', orderBy = 'postId')
 */
class Mpay24Cw_Entity_Transaction extends Customweb_Payment_Entity_AbstractTransaction {
	...
	public function getOrder(){
		...
		if(empty($orderPostId)) {
			return Mpay24Cw_Util::loadOrderObjectById($this->getOrderId());
		}
		return Mpay24Cw_Util::loadOrderObjectById($orderPostId);
	...
	protected function updateOrderStatus(Customweb_Database_Entity_IManager $entityManager, $orderStatus, $orderStatusSettingKey){
		...
		$order = $this->getOrder();
		$paymentMethod = Mpay24Cw_Util::getPaymentMehtodInstance($this->getPaymentClass());
		if ($orderStatusSettingKey != 'status_authorized' || $paymentMethod->getPaymentMethodConfigurationValue('status_authorized') != 'use-default') {
			$order->update_status($orderStatus, __('Payment Notification', 'woocommerce_mpay24cw'));
		}
	}
	...
	protected function authorize(Customweb_Database_Entity_IManager $entityManager){
		if ($this->getTransactionObject()->isAuthorized()) {
			...
			$GLOBALS['woo_mpay24cwAuthorizeLanguage'] = $this->getTransactionObject()->getTransactionContext()->getOrderContext()->getLanguage()->getIetfCode();
			add_filter('locale', 'woocommerce_mpay24cw_locale');
			...
			remove_filter('locale', 'woocommerce_mpay24cw_locale');
		}
	}
	...
	protected function forceTransactionFailing(){
		...
		if(Mpay24Cw_Util::getAuthorizedTransactionByPostId($this->getPostId()) !== null){
			//Another Transaction has already sucessfuly authorized this transaction, do not mark the order as cancelled
			return;
		}
		$this->getOrder()->add_order_note(__('Error Message: ', 'woocommerce_mpay24cw') . $message);
		...
		if (class_exists('WC_Subscriptions') && version_compare(WC_Subscriptions::$version, '2.0') >= 0) {
			if ($this->getAuthorizationType() != Customweb_Payment_Authorization_Recurring_IAdapter::AUTHORIZATION_METHOD_NAME) {
				if($this->getTransactionObject()->getTransactionContext()->createRecurringAlias()) {
					...
					add_action( 'woocommerce_email', 'woocommerce_mpay24cw_unhook_subscription_cancel' );
				}
