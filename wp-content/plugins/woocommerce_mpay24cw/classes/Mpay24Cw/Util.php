...
require_once 'Mpay24Cw/Entity/PaymentCustomerContext.php';
...
require_once 'Mpay24Cw/ContextCleanUpBean.php';
...
require_once 'Mpay24Cw/Database/Driver.php';
require_once 'Customweb/Util/Html.php';
require_once 'Customweb/Database/Migration/Manager.php';
require_once 'Customweb/Asset/Resolver/Simple.php';
require_once 'Mpay24Cw/LayoutRenderer.php';
require_once 'Mpay24Cw/EntityManager.php';
...
class Mpay24Cw_Util {
	...
	public static function getPluginUrl($controller, array $params = array(), $action = null){
		...
		$url = new Customweb_Core_Url(get_permalink(get_option('woocommerce_mpay24cw_page')));
		...
		return apply_filters('woocommerce_mpay24cw_plugin_url', $complete, $url->getBaseUrl() . $url->getPath(), 
				$url->getQueryAsArray());
	}
	...
	public static function installPlugin(){
		...
		$manager = new Customweb_Database_Migration_Manager(self::getDriver(), dirname(__FILE__) . '/Migration/', 
				$wpdb->prefix . 'woocommerce_mpay24cw_schema_version');
		$manager->migrate();
		
		//Create Page
		$optionValue = get_option('woocommerce_mpay24cw_page');
		$pageContent = '[woocommerce_mpay24cw]';
		$pageSlug = 'woo_mpay24cw';
		$pageTitle = 'mPAY24 Checkout';
		...
		update_option('woocommerce_mpay24cw_page', $pageId);
		
		// Append order status for pending payments
		if (!term_exists('mpay24cw-pending', 'shop_order_status')) {
			wp_insert_term('mpay24cw-pending', 'shop_order_status',
					array(
						'description' => 'Orders with that order status are currently in the checkout of mPAY24.',
						'slug' => 'mpay24cw-pending'
					));
		}
	}
	...
	public static function includeTemplateFile($templateName, $variables = array()){
		...
		$templateName = 'mpay24cw_' . $templateName;
	...
	public static function createContainer(){
		if (self::$container === null) {
			...
			// @formatter:off
			$storage = new Customweb_Storage_Backend_Database(self::getEntityManager(), self::getDriver(), 'Mpay24Cw_Entity_Storage');
			$provider->addObject(Mpay24Cw_ContextRequest::getInstance())
				->addObject(self::getEntityManager())
				->addObject(self::getDriver())
				->addObject(new Mpay24Cw_LayoutRenderer())
				->addObject(new Customweb_Cache_Backend_Memory())
				->add('databaseTransactionClassName', 'Mpay24Cw_Entity_Transaction')
				->addObject(self::getAssetResolver())
				->addObject($storage)
				->addObject(new Mpay24Cw_ContextCleanUpBean(self::getEntityManager()))
				->addObject(new Mpay24Cw_TransactionCleanUpBean(self::getEntityManager()));
			// @formatter:om
	...
	public static function getEntityManager(){
		if (self::$entityManager === null) {
			...
			self::$entityManager = new Mpay24Cw_EntityManager(self::getDriver(), $cache);
		}
	...
	/**
	 *
	 * @return Mpay24Cw_Database_Driver
	 */
	public static function getDriver(){
		if (self::$driver === null) {
			global $wpdb;
			$wpdb->hide_errors();
			self::$driver = new Mpay24Cw_Database_Driver($wpdb);
		}
		return self::$driver;
	}
	...
	public static function getPaymentCustomerContext($customerId){
		...
		if (!isset(self::$paymentCustomerContexts[$customerId])) {
			$entities = self::getEntityManager()->searchByFilterName('Mpay24Cw_Entity_PaymentCustomerContext', 'loadByCustomerId', 
					array(
			...
			if (count($entities) > 0) {
				...
				$context = new Mpay24Cw_Entity_PaymentCustomerContext();
	...
	public static function persistPaymentCustomerContext(Customweb_Payment_Authorization_IPaymentCustomerContext $context){
		if ($context instanceof Mpay24Cw_Entity_PaymentCustomerContext) {
	...
	public static function thankYouPageHtml($orderId){
		...
		if (!empty($paymentInformation)) {
			echo '<div class="woocommerce_mpay24cw-payment-information" id="woocommerce_mpay24cw-payment-information">';
			echo "<h2>" . __('Payment Information', 'woocommerce_mpay24cw') . "</h2>";
	...
	public static function orderEmailHtml($order, $sent_to_admin, $plain_text = false){
		...
		$paymentInformation = $transactionObject->getPaymentInformation();
		if(!empty($paymentInformation)) {
			echo '<div class="woocommerce_mpay24cw-email-payment-information" id="woocommerce_mpay24cw-email-payment-information">';
			echo "<h2>" . __('Payment Information', 'woocommerce_mpay24cw') . "</h2>";
			echo $transactionObject->getPaymentInformation();
			echo '</div>';
		}
	 ...
	 * @return Mpay24Cw_Entity_Transaction The matching transactions for the given transaction id
	 */
	public static function getTransactionById($id, $cache = true){
		return self::getEntityManager()->fetch('Mpay24Cw_Entity_Transaction', $id, $cache);
	}
	 ...
	 * @param boolean $cache load from cache
	 * @return Mpay24Cw_Entity_Transaction The matching transactions for the given transactionNumber
	 */
	public static function getTransactionByTransactionNumber($number, $cache = true){
		$transactions = self::getEntityManager()->searchByFilterName('Mpay24Cw_Entity_Transaction', 'loadByExternalId', 
				array(
	 ...
	 * @param boolean $cache load from cache
	 * @return Mpay24Cw_Entity_Transaction[] The matching transactions for the given order id
	 */
	public static function getTransactionsByOrderId($orderId, $cache = true){
		class_exists('WC_Order');
		self::getPaymentMethods(true);
		return self::getEntityManager()->searchByFilterName('Mpay24Cw_Entity_Transaction', 'loadByOrderId', 
				array(
					'>orderId' => $orderId 
				), $cache);
	}
	...
	public static function getTransactionsByPostId($postId, $cache = true){
		class_exists('WC_Order');
		self::getPaymentMethods(true);
		return self::getEntityManager()->searchByFilterName('Mpay24Cw_Entity_Transaction', 'loadByPostId',
				array(
					'>postId' => $postId
				), $cache);
	}
	...
	public static function getAliasTransactions($userId, $paymentMethod){
		...
		$aliases = array();
		$entities = self::getEntityManager()->search('Mpay24Cw_Entity_Transaction', 
...
;
	...
	public static function computeTransactionValidateHash(Mpay24Cw_Entity_Transaction $transaction) {
...
if(function_exists('wc_get_page_id')) {
	...
	$id = Mpay24Cw_Util::getShopOption('woocommerce_checkout_page_id');
